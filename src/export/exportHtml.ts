...
import { breakpointDirection, sortBreakpoints } from '../model/factory';
...
function fluidGeometry(
  ...
  css.left = percent(geom.x, layoutWidth);
  css.top = percent(geom.y, layoutHeight);
  css.width = percent(geom.width, layoutWidth);
  css.height = percent(geom.height, layoutHeight);
}
...
function layoutExtent(
  ...
  return {
    width: Math.max(minimumWidth, ...components.map((cmp) => Math.max(0, cmp.x + cmp.width))),
    height: Math.max(minimumHeight, ...components.map((cmp) => Math.max(0, cmp.y + cmp.height))),
  };
}
...
function buildStylesheet(project: Project): string {
  ...
  // Reset-ish base shared by every page. Page fills the viewport from the left
  // edge so canvas X/Y map 1:1 to screen pixels (no auto-centering gutter).
  lines.push('/* Generated by SiteBuilder — shared stylesheet */');
  lines.push('html, body { margin: 0; width: 100%; max-width: 100%; overflow-x: hidden; }');
  lines.push('body { font-family: Arial, sans-serif; }');
  lines.push('.sb-page { position: relative; margin: 0; box-sizing: border-box; }');
  ...
  // Per-page wrapper rules. No breakpoints: fill the viewport (min-width = authored
  // width) left-anchored so canvas X/Y map 1:1. With breakpoints: bound the page
  // to its authored width and let the named @media rules drive narrower layouts.
  const hasMaxBp = project.breakpoints.some((bp) => breakpointDirection(bp) === 'max');
  for (const page of project.pages) {
    const baseWidth = responsiveBaseWidth(page);
    const widthRule = hasMaxBp
      ? `width: 100%; max-width: ${px(baseWidth)};`
      : `width: 100%; min-width: ${px(baseWidth)};`;
    lines.push(
      `.${pageClassName(page)} { ${widthRule} min-height: ${px(page.height)}; background: ${page.backgroundColor || '#ffffff'}; }`
    );
  }
  lines.push('');
  ...
  for (const page of project.pages) {
    ...
    for (const cmp of page.components) {
      if (cmp.hidden || cmp.props?.protected) continue; // hidden/protected components are excluded entirely
  ...
  // Default responsive narrowing only when named breakpoints exist (canvas ==
  // screen stays px-exact otherwise). Scales components to fluid percentages
  // between the authored width and the widest breakpoint.
  if (hasMaxBp) {
    for (const page of project.pages) {
      const visible = page.components.filter((cmp) => !cmp.hidden && !cmp.props?.protected);
      const extent = layoutExtent(visible, responsiveBaseWidth(page), page.height);
      lines.push(`@media (max-width: ${extent.width}px) {`);
      lines.push(
        `  .${pageClassName(page)} { width: 100%; max-width: 100%; min-height: 0; height: auto; aspect-ratio: ${extent.width} / ${extent.height}; }`
      );
      for (const cmp of visible) {
        const css: Record<string, string> = {};
        fluidGeometry(css, cmp, extent.width, extent.height);
        lines.push(`  #${elementId(cmp)} { ${cssDeclarations(css)} }`);
      }
      lines.push('}');
      lines.push('');
    }
  ...
  // Named breakpoints: max-width rules are widest first, min-width rules are
  // narrowest first, so the CSS cascade naturally applies the closest match.
  const bps = sortBreakpoints(project.breakpoints);
  const previousId: Record<'max' | 'min', string | null> = { max: null, min: null };
  for (const bp of bps) {
    const direction = breakpointDirection(bp);
    const inner: string[] = [];
    ...
    for (const page of project.pages) {
      const baseWidth = responsiveBaseWidth(page);
      const resolvedComponents = page.components
        .filter((cmp) => !cmp.hidden && !cmp.props?.protected)
        .map((cmp) => resolveComponent(cmp, bps, bp.id, baseWidth));
      const visibleResolved = resolvedComponents.filter(
        (cmp) => !resolveComponentHidden(cmp, bps, bp.id)
      );
      const extent = layoutExtent(visibleResolved, bp.maxWidth, page.height);
      if (direction === 'max') {
        inner.push(
          `  .${pageClassName(page)} { width: 100%; max-width: 100%; min-height: 0; height: auto; aspect-ratio: ${extent.width} / ${extent.height}; }`
        );
        ...
        inner.push(
          `  .${pageClassName(page)} { width: 100%; max-width: none; min-height: 0; height: auto; aspect-ratio: ${extent.width} / ${extent.height}; }`
        );
      }
      for (const cmp of page.components) {
        if (cmp.hidden || cmp.props?.protected) continue; // base-hidden/protected components are excluded entirely
        const elId = elementId(cmp);
        const resolved = resolveComponent(cmp, bps, bp.id, baseWidth);
        const hidden = resolveComponentHidden(cmp, bps, bp.id);
        const aboveHidden = previousId[direction] ? resolveComponentHidden(cmp, bps, previousId[direction]) : !!cmp.hidden;
        if (hidden) {
        ...
        fluidGeometry(css, geom, extent.width, extent.height);
    ...
    previousId[direction] = bp.id;
    const orientation =
      bp.orientation && bp.orientation !== 'none' ? ` and (orientation: ${bp.orientation})` : '';
    const mediaFeature = direction === 'min' ? 'min-width' : 'max-width';
    lines.push(`@media (${mediaFeature}: ${bp.maxWidth}px)${orientation} {`);
    lines.push(...inner);
    lines.push('}');
    lines.push('');
  }

  // Keep the fluid page constrained while media queries provide the layout.
  lines.push('.sb-fit { width: 100%; max-width: 100%; overflow-x: hidden; }');
  lines.push('');
  ...
  // Keep the document scrollbar contract authoritative even when custom page
  // CSS is appended above.
  lines.push('html, body { overflow-x: hidden !important; }');
  lines.push('.sb-fit { width: 100% !important; max-width: 100% !important; overflow-x: hidden !important; }');
...
/**
 * Static snapshot HTML for merged objects (store mergeSelection): each source
 * component absolutely positioned with inline styles inside a relative wrapper
 * whose origin is the selection bounding box. `effs[i]` is the effective
 * (breakpoint-resolved) component for `items[i]`.
 */
export function mergedBlockHtml(items: ComponentItem[], effs: ComponentItem[]): string {
  ...
  for (let i = 0; i < items.length; i++) {
    const cmp = items[i];
    const eff = effs[i];
    const css = componentCss(cmp, { x: 0, y: 0, width: eff.width, height: eff.height }, eff.props);
    // The wrapper below carries the geometry; the element itself fills it.
    delete css.position;
    delete css.left;
    delete css.top;
    css.width = '100%';
    ...
    const inner = componentHtml(cmp, []);
    const styled = inner.replace(/^(<[a-zA-Z][^>]*?)>/, `$1 style="${escapeAttr(cssDeclarations(css))}">`);
    parts.push(
      `<div style="position:absolute;left:${px(eff.x - minX)};top:${px(eff.y - minY)};width:${px(eff.width)};height:${px(eff.height)};">${styled}</div>`
    );
  }
  return parts.join('');
}
...
function buildPageHtml(page: Page): string {
  ...
  lines.push(`<div class="sb-fit sb-fit-${name}"><div class="sb-page ${pageClassName(page)}" id="page-${name}">`);
