...
function buildStylesheet(project: Project): string {
  ...
  lines.push('.sb-page { position: relative; margin: 0 auto; box-sizing: border-box; }');
  ...
  for (const page of project.pages) {
    ...
    lines.push(
      `.${pageClassName(page)} { width: 100%; max-width: ${px(baseWidth)}; min-height: ${px(page.height)}; background: ${page.backgroundColor || '#ffffff'}; }`
    );
  }
  lines.push('');
  ...
  // Make the Default layout fluid below its authored width. This covers every
  // arbitrary viewport between Desktop and the widest named breakpoint.
  for (const page of project.pages) {
  ...
  // Named breakpoints are max-width rules, widest first so narrower layouts
  // override wider layouts naturally through the CSS cascade.
  ...
  for (const bp of bps) {
    ...
    lines.push(`@media (max-width: ${bp.maxWidth}px)${orientation} {`);
    lines.push(...inner);
    lines.push('}');
    lines.push('');
  }
