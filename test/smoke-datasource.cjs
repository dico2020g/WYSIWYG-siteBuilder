...
app.whenReady().then(async () => {
  ...
  // Seed a saved connection directly in the store
  await js(`(() => {
    // find store via React: use the connections modal flow instead is slow — inject via localStorage not possible.
    // Use UI: open connections modal, create, save.
  ...
  await sleep(400);
  await shot('smoke-ds-1-browser.png');
  ...
  await sleep(1500);
  await shot('smoke-ds-2-database.png');

  // Expand Tables category
  await js(`[...document.querySelectorAll('.ds-node')].find(n => n.textContent.includes('Tables')).click();`);
  await sleep(300);
  // Expand a table's fields
  await js(`[...document.querySelectorAll('.ds-node')].find(n => n.textContent.trim().startsWith('▦ users') || n.textContent.includes('users')).click();`);
  await sleep(1200);
  await shot('smoke-ds-3-tables.png');

  // Double-click users table → open data grid
  await js(`(() => {
    const n = [...document.querySelectorAll('.ds-node')].find(n => n.textContent.includes('users') && !n.textContent.includes('Fields'));
    n.dispatchEvent(new MouseEvent('dblclick', { bubbles: true }));
  })()`);
  await sleep(1200);
  await shot('smoke-ds-4-open-table.png');

  // Context menu on a table in tree
  await js(`(() => {
